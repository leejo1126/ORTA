...
def build_panel(cfg: Config, marker: str, fov: int = 0, n: int = 6,
    ...
    return Panel(marker, fov, cells, raw, nuc, slices, snr)
...
def proxy_metrics(per_cell: list[dict], image_snr: float) -> dict:
    """Aggregate per-cell ingredients into proxy metrics + a single agnostic score in
    [0,1] (higher = more foci-like), SNR-aware. Also returns a `valid` gate."""
    ...
    # SNR-aware weights: at low panel SNR, lean on contrast + reproducibility
    low = image_snr < 8.0
    ...
    if not valid:
        score *= 0.05
    return {
        "score": round(score, 4), "valid": bool(valid),
        "median_count": median_count, "count_cv": round(count_cv, 3),
        "median_fill": round(median_fill, 4), "median_contrast": round(median_contrast, 2),
        "median_vol": median_vol, "frac_tiny": round(frac_tiny, 3),
        "frac_huge": round(frac_huge, 3), "image_snr": round(image_snr, 2),
    }
...
def run_spec(panel: Panel, spec: Spec, out_png: str | None = None, mag: int = 4) -> dict:
    ...
    metrics = proxy_metrics(per_cell, panel.image_snr)
